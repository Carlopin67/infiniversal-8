...
const Install = (() => {
  ...
  let promptResolvers = []; // callbacks esperando el prompt
  ...
  // ── Captura del prompt nativo ─────────────────────────────────
  // Chrome/Android lo emite cuando la PWA cumple los criterios.
  // Lo guardamos y notificamos a quien esté esperando.
  window.addEventListener('beforeinstallprompt', e => {
    e.preventDefault();
    deferredPrompt = e;
    console.log('[Install] beforeinstallprompt capturado');
    // Notificar a todos los que estaban esperando
    promptResolvers.forEach(r => r(e));
    promptResolvers = [];
    updateSettingsState();
  });
  ...
  window.addEventListener('appinstalled', () => {
    ...
    updateSettingsState();
    console.log('[Install] App instalada');
  });

  // Devuelve el prompt si ya está disponible, o espera hasta 8s
  function waitForPrompt() {
    if (deferredPrompt) return Promise.resolve(deferredPrompt);
    return new Promise(resolve => {
      promptResolvers.push(resolve);
      setTimeout(() => {
        // Si no llegó en 8s, resolver con null
        const idx = promptResolvers.indexOf(resolve);
        if (idx !== -1) { promptResolvers.splice(idx, 1); resolve(null); }
      }, 8000);
    });
  }

  // ── Lanzar el diálogo nativo ──────────────────────────────────
  ...
  // ── Modal principal ───────────────────────────────────────────
  // Se muestra siempre al abrir la app si no está instalada.
  // Si el prompt aún no llegó, el botón espera en segundo plano.
  async function showInstallModal() {
    if (isInstalled()) return;
    // No repetir si ya la instaló o la descartó en esta sesión
  ...
  btnAccept.addEventListener('click', async () => {
    if (isIOS) return; // iOS: instrucciones manuales en el modal

    if (deferredPrompt) {
      // Prompt listo: lanzar instalación nativa — abre el diálogo del sistema
      const ok = await triggerPrompt(btnAccept);
      modalEl.classList.add('hidden');
      if (ok) hideBanner();
    } else {
      // Prompt aún no llegó: esperar hasta 10s en silencio
      const orig = btnAccept.textContent;
      btnAccept.textContent = '⏳ Preparando...';
      btnAccept.disabled = true;

      const prompt = await waitForPrompt();

      if (prompt) {
        btnAccept.textContent = orig;
        btnAccept.disabled = false;
        const ok = await triggerPrompt(btnAccept);
        modalEl.classList.add('hidden');
        if (ok) hideBanner();
      } else {
        // Primera visita: Chrome aún no tiene el prompt listo.
        // En la próxima visita ya funcionará directamente.
        modalEl.classList.add('hidden');
  ...
  // ── Banner discreto (tras descartar el modal) ─────────────────
  function showBanner() {
  ...
  // Inicialización al arrancar: siempre mostrar modal si no está instalada
  function initBanner() {
    if (isInstalled()) return;
    // Mostrar modal inmediatamente (con pequeño delay para que la UI esté lista)
    setTimeout(() => showInstallModal(), 400);
  }

  // maybeShowModal: después del tutorial, también mostrar el modal
...
const PoemStructures = {
  soneto: {
    ...
    desc: 'La cima de la lírica occidental. Dos cuartetos de rima ABBA seguidos de dos tercetos encadenados. Endecasílabos (11 sílabas).',
    stanzas: [
  ...
  cuarteto: {
    ...
    desc: 'Cuatro versos endecasílabos con rima abrazada ABBA. La estrofa más noble y equilibrada de la métrica española.',
  ...
  lira: {
    ...
    desc: 'Cinco versos alternando heptasílabos (7) y endecasílabos (11) con rima aBabB. Popularizada por Garcilaso de la Vega.',
  ...
  haiku: {
    ...
    desc: 'Tres versos que capturan un instante de la naturaleza o del tiempo. Origen japonés. Sin rima. Solo imagen y silencio.',
  ...
  silva: {
    ...
    desc: 'Combinación libre de versos de 7 y 11 sílabas, con rima a elección del poeta. Máxima libertad dentro de la métrica clásica.',
  ...
  decima: {
    ...
    desc: 'Diez versos octosílabos con rima abbaaccddc. También llamada Espinela. Base del verso popular latinoamericano y del repentismo.',
  ...
  oda: {
    ...
    desc: 'Composición lírica de tono elevado, dividida en estrofas simétricas. Permite exaltar personas, ideas o sentimientos con libertad métrica.',
    stanzas: [
  ...
  libre: {
    ...
    desc: 'Sin métrica fija, sin rima obligatoria. El ritmo lo decides tú: con el corte de verso, el silencio y la repetición de imágenes.',
...
const ListView = (() => {
  ...
  function render() {
    ...
    // Mostrar hint de selección si hay al menos 2 notas
    if (selectHintEl) {
  ...
  function createCard(note) {
    ...
    card.querySelector('.note-card-fav').addEventListener('click', e => {
      if (BulkSelect.isActive()) return; // En modo selección no tocar favorito
      e.stopPropagation();
  ...
  function toggleCardSelect(card, id) {
    ...
    card.classList.toggle('selected', card.classList.contains('selected') === false
      ? true : false);
    // Re-evaluar desde el set real de BulkSelect
...
const NewNoteFlow = (() => {
  ...
  function renderPreview(key) {
    ...
    if (key === 'libre') {
      createBtn.style.background = 'var(--accent-2)';
    } else {
  ...
  // Inicializar con soneto seleccionado
  ...
  typeModal.querySelectorAll('.type-btn').forEach(btn =>
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.type === 'song') {
        ...
        // Resetear selección al soneto al abrir
...
/* ════════════════════════════════════════
   MÓDULO: BULK SELECT
   Selección múltiple de notas para
   eliminar o compartir varias a la vez.
════════════════════════════════════════ */
const BulkSelect = (() => {
...
const BulkDeleteModal = (() => {
  ...
  function open(selectedIds, cb) {
    ...
    subtitle.textContent = n === 1
      ? 'Esta acción no se puede deshacer. ¿Seguro que quieres eliminar esta nota?'
      : `Esta acción no se puede deshacer. ¿Seguro que quieres eliminar ${n} notas?`;
    overlay.classList.remove('hidden');
  }
...
(function init() {
  Settings.init();
  ListView.render();
  Install.initBanner(); // Banner + modal de instalación en móvil

  if (Storage.isFirstRun()) {
    Tutorial.show();
    // Nota: el prompt de instalación se mostrará DESPUÉS del tutorial
    // via Tutorial.hide() → Install.maybeShowModal()
  }
  // No llamamos maybeShowModal() aquí porque initBanner ya lo gestiona
  // para iOS, y beforeinstallprompt lo gestiona para Android/Chrome
})();
